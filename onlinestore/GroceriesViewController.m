...
- (void)viewDidLoad
{
    ...
    //[self setHeaderView:NO];
    //self.navigationController.navigationBarHidden = NO;
    //self.edgesForExtendedLayout = UIRectEdgeNone;
    //[self.view addSubview:WishDisplayViewController];
    //[self dismissViewControllerAnimated:YES completion:nil];

    // self.navigationController.navigationBarHidden = YES;
    
    [_myScroolObj setScrollEnabled:YES];
    [_myScroolObj setContentSize:CGSizeMake(320, 1000)];
    arrObj = [[NSArray alloc]initWithObjects:@"weekly",@"monthly",@"quarterly",@"half-yearly",@"once",@"when finds a good deal" ,nil];
...
-(void)dropDownBtnMethods
{
   ...
   // table = [UITableView alloc]initWithFrame:CGRectMake(<#CGFloat x#>, <#CGFloat y#>, <#CGFloat width#>, <#CGFloat height#>) style:<#(UITableViewStyle)#>;
    
    table.delegate = self;
    table.dataSource = self;
    [_myScroolObj addSubview:table];
    [table reloadData];
    table.hidden=YES;
}
...
- (IBAction)browseBtn:(id)sender {
    ...
    WishDisplayViewController *disp = [self.storyboard instantiateViewControllerWithIdentifier:@"WishDisplayViewController"];
    [self presentViewController:disp animated:YES completion:nil];
    //[self.navigationController pushViewController:disp animated:NO];
